    ...
    <Modal show={show} onHide={handleClose}>
      ...
      <Modal.Body>
          ...
          {(availableStages || [])
            .filter(
              (stage) =>
                !stages.some(
                  (s) =>
                    s.stage_id === stage.stage_id && s.status === "completed"
                )
            )
            .map((stage) => (
              <div key={stage.stage_id} className="mb-3 border p-2 rounded">
                <Form.Check
                  ...
                  checked={
                    !!selectedStages.find((s) => s.stage_id === stage.stage_id)
                  }
                />
                <Form.Control
                  type="date"
                  ...
                  onChange={(e) =>
                    handleStageSelection(stage.stage_id, "date", e.target.value)
                  }
                />
                <Form.Check
                  type="radio"
                  ...
                  onChange={() =>
                    handleStageSelection(stage.stage_id, "status", "ongoing")
                  }
                />
                <Form.Check
                  type="radio"
                  ...
                  onChange={() =>
                    handleStageSelection(stage.stage_id, "status", "completed")
                  }
                />
              </div>
            ))}
        ...
        <Form.Check
          ...
          onChange={() =>
            setCustomStage({ ...customStage, status: "completed" })
          }
        />
      </Modal.Body>
      <Modal.Footer>
